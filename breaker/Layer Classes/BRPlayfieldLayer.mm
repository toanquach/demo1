...
-(id) init
{
    if (self = [super init])
    {
        ...
        bricksheet = [CCSpriteBatchNode batchNodeWithFile:@"bricksheet.png" capacity:100];
        
        // Add the batch node to the layer
        [self addChild:bricksheet z:1];
        
        // Add the background
        CCSprite *bg = [CCSprite spriteWithSpriteFrameName:@"brick_bg.png"];
        ...
        // Build the paddle
        [self buildPaddleAtStartingPosition:ccp(size.width/2,75)];
        
        // Load the level patterns
        patternDefs = [NSDictionary dictionaryWithDictionary:[gh getDictionaryFromPlist:@"patterns"]];
        
        // Load the brick pattern
        NSInteger uniquePatterns = 4;
        NSInteger newPattern =( [gh currentLevel] -1)% uniquePatterns;
...
{
    // Create the paddle
    paddle = [PhysicsSprite spriteWithSpriteFrameName:@"paddle.png"];
    ...
    paddleBodyDef.position.Set(startPos.x/PTM_RATIO,startPos.y/PTM_RATIO);
